...
class GPUFluid{
	...
	public function new(width:Int, height:Int, cellSize:Float = 8, solverIterations:Int = 18){
		...
		//use float fields where possible (to prevent errors with unsigned byte fields)
		floatVelocity = 
		floatPressure =  
		floatDivergence = floatDataType != null;
		//always use unsigned byte dye field
		floatDye = false;//(floatDataType != null) && floatLinearFilter;
